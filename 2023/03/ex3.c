...
int get_adjacents_old(int *nvals, int *nidx, int *sidx, int n_num, 
                      int n_sym,  int *hvals, int *hidx) {
...
int search_buff(char lbuff[3][MAX_LEN], 
    ...
    // get number info for row = 1
    n_num = find_numbers(lbuff[1], iarr, varr);

   
    // get symbols for each line and search for hits
    for(ii=0; ii<3; ii++){
        n_sym = find_symbols(lbuff[ii], siarr, svarr);
        // will add 1 to hits array at num index
        get_adjacents(varr, iarr, siarr, n_num, n_sym,  hits);
    }
